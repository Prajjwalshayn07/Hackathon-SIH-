// Civic Issue Reporter with Firebase Integration
...
async function handleFormSubmit(e) {
    ...
    try {
        // Submit to Firebase
        const issueId = await submitIssueToFirestore(issueData);
        
        // Show success modal
        showSuccessModal(issueId);
        
        // Reset form
        document.getElementById('reportForm').reset();
        document.getElementById('imagePreview').innerHTML = '';
        
        // Reload issues and statistics
        loadIssues();
        loadStatistics();
    } catch (error) {
        ...
        <div class="col-md-6 col-lg-4 mb-3">
            <div class="card issue-card ${issue.status}">
                ...
                <div class="card-body">
                    ...
                    <p class="card-text">
                        <small class="text-muted">
                            <i class="fas fa-clock"></i> ${formatDate(issue.createdAt)}
                        </small>
                    </p>
                </div>
            </div>
        ...
        if (issue.latitude && issue.longitude) {
            ...
            marker.bindPopup(`
                <div>
                    ...
                    <small><i class="fas fa-clock"></i> ${formatDate(issue.createdAt)}</small>
                </div>
            `);
...
async function loadStatistics() {
    try {
        const stats = await getStatisticsFromFirestore();
        ...
        document.getElementById('resolvedIssues').textContent = stats.resolved || 0;
    } catch (error) {
